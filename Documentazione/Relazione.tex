...
\begin{description}
  \item[Main Thread (producer)]: Ascolta sulla FIFO pubblica, gestisce le \texttt{REQ\_QUERY} e le \texttt{REQ\_CALC}. Le prime vengono gestite immediatamente (interrogazione alla cache). Per le secondo viene prima verificata la presenza in cache, se il file non è presente viene inserito un nuovo task nella coda condivisa e viene attivato un worker.
...
\begin{figure}[H]
    ...
    \caption{Schema logico del meccanismo di accodamento e schedulazione (Shortest Job First).}
\end{figure}
...
A differenza della richiesta, la risposta viaggia su un canale dedicate per evitare \textit{race conditions} in lettura tra client diversi. é stato necessario sincronizzare l'apertura della FIFO privata lato server (che deve avvenire solo dopo che il client l'ha creata) e gestire correttamente la scrittura bloccante, assicurando che ogni PID ricevesse esclusivamente il proprio hash calcolato.

\subsection{Cache querying}
